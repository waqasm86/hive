...
- **aden_tools** - 에이전트 기능을 위한 106개의 MCP 도구
- 필요한 모든 의존성
...
├── tools/                  # MCP 도구 패키지 – 에이전트 기능을 위한 106개 도구
...
Aden은 다음과 같은 포괄적인 관측성 기능을 제공합니다. 실시간 에이전트 실행 모니터링을 위한 WebSocket 스트리밍, TimescaleDB 기반의 비용 및 성능 메트릭 분석, Kubernetes 연동을 위한 헬스 체크 엔드포인트, 예산 관리, 에이전트 상태, 정책 제어를 위한 106개의 MCP 도구
